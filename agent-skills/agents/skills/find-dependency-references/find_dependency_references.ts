...
function loadGitignore(): Set<string> {
  const gitignorePath = path.join(KIBANA_ROOT, '.gitignore');
...
function shouldIgnoreFile(filePath: string, gitignorePatterns: Set<string>): boolean {
  const relativePath = path.relative(KIBANA_ROOT, filePath);
...
function analyzeDependency(dependencyName: string): AnalysisResult {
  ...
  const allFiles = getAllFiles(KIBANA_ROOT, gitignorePatterns);
  ...
  const filesByTeamResult: TeamGroup[] = Array.from(filesByTeam.entries())
    ...
    .map(([team, files]) => ({
      ...
      files: files.map((filePath) => {
        ...
        return {
          filePath: path.relative(KIBANA_ROOT, filePath),
          apis: usage.apis,
        };
      }),
    }));
  ...
  return {
    ...
    matchingFiles: sortedFiles.map((file) => path.relative(KIBANA_ROOT, file)),
